...
Download the lecture slides [here](data/Wk8/Two-Factor Mixed and Within-Participants ANOVA HANDOUT Updated.pdf), and [here](data/Wk8/Two-Factor Mixed and Within-Participants ANOVA Updated.pdf) for a larger version. -->
...
mixedDescriptives = stroopMixedLong %>%
  ...
  # Print the results
  print.data.frame(mixedDescriptives)  
```
:::
...
withinDescriptives = stroopWithinLongSep %>%
  ...
  get_summary_stats(RT, show = c("mean","sd","ci"))
  # Print the results
  print.data.frame(withinDescriptives)
```
