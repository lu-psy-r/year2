...
-   The first folder is a zipped folder called `Lecture_Week_13_Materials.zip`.
    It can be downloaded [here](data/Wk3/Lecture_Week_13_Materials.zip). This has the
    script and the `KidIQ.txt` file for the model that was demonstrated in the lecture
-   The second folder is called `Lab_Week_13_Materials.zip`. In this folder is the 
    `WorldHappinessReportData.csv` and a half prepared .Rmd file for the lab in week 13.  I did this because it is a really interesting dataset but the tidying and visualisation would take forever....please download it [here](data/Wk3/Lab_Week_13_Materials.zip).
    
## Pre-Lab Activity

1.  Watch the lecture and play with the lecture script before you attempt anything!
...
The model output has several predictors with suffixes, demarcated by an
...
The link to this week's multiple choice quiz, WBA 3, is [here](https://modules.lancaster.ac.uk/mod/quiz/view.php?id=2738522)
...
```{r}
d <- read_csv("data/Wk3/Lecture_Week_13_Materials/KidIQ.txt", col_types = cols(mom_work = col_skip()))
...
Many effect sizes for independent predictors (or main effects in ANOVA language) 
in psychology hover around Cohen’s *d* = 0.4
