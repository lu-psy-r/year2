...
One way to mitigate multicollinearity is to simple leave some variables out of your dataset - and there are estimation practices and methods that allow you to choose which to leave out in a principled, reproducible way.  
   
   Another way to mitigate multicollinearity is not to leave out variables but to combine variables that are highly correlated together into one new variable. This is what a factor analysis method does.  It calculates what information is unique in the predictor for a hypothesised new variable and takes only sufficient information from each observed variables that uniquely contributes to the new variable. In this way, the colinearity is reduced in the dataset but the relevant information from the observed datasets is retained.  
   
   Theory should suggest which variables are likely to "load" together onto a latent variable.  However, you can also take a data driven approach (by letting the model suggest loadings) if theory is agnostic, or variables are quite new.
...
There are some conditions associated with making the new latent variables that must be met: 
 ...
 - Correlations between those three or more observed variables should be high.
