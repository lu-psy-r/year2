...
There are two folders to download for week 12:  I separated the materials into files tha R will upload, or should upload with no error; and .docx or .pdf files that R would not like.  Please store the second folder somewhere for your use.
 ...
 - The first folder is a zipped folder called Week 12 234 Materials.zip. It can be downloaded [here](data/Wk2/Week_12_234_Materials.zip). You can upload the zip file directly on to the R server and it will populate a new folder with the files and data files automatically.
 - The second folder is called "Week_12_234_Slides_Codebook". The slides are for the lecture; the Codebook is paired with the "student-por.csv" dataset, for the pre-lab and lab activity is available in a folder for download [here](data/Wk2/Week_12_234_Slides_Codebook).
...
## Pre-Lab Activity
  ...
  6. Start a script and import the dataset.  Use the lecture and the model script `Multiple_Regression_Categorical_Predictors_2025.Rmd` in the `Week 12 lecture materials` folder to think about what processes may you need to perform to get to a model that looks like the output.
...
The link to this week's multiple choice quiz, WBA 2, is [here](https://modules.lancaster.ac.uk/mod/quiz/view.php?id=2506929)
...
In this lab, you will be working with the R script, data and Codebook stored [here](Week_12_234_Materials.zip_materials.zip), and the accompanying codebook is accessed [here](data/Wk2/Data_Codebook.pdf).
