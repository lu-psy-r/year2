...
For every one point increase in Mother's IQ, a child's test score increases by 0.61 of a point (*t* = 10.42, *p* < .001). PP
...
The model plot, with the addition of the interaction between mother's IQ and high school education, looks like this: I have recycled old code here, based in a model with the age variable left in.  Don't worry about the model here. Just concentrate on the plot

```{r}
m3_c_int_age <- lm(kid_score ~ mom_hs + mom_age + mom_iq + mom_hs:mom_iq, d)
```

```{r}
(p_int <- ggplot(d, aes(mom_iq, kid_score)) +
  geom_point(aes(color = factor(mom_hs)), show.legend = FALSE) +
  geom_abline(
    intercept = c(coef(m3_c_int_age)[1], sum(coef(m3_c_int_age)[1:2])),
    slope = c(coef(m3_c_int_age)[4], sum(coef(m3_c_int_age)[4:5])),
    color = c("black", "grey")) +
